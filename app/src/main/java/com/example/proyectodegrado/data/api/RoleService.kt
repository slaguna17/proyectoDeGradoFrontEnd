...
interface RoleService {
    ...
    suspend fun updateRole(@Path("id") roleId: Int, @Body request: RoleRequest)

    //Delete Role
    @DELETE("/api/roles/deleteRole/{id}")
    suspend fun deleteRole(@Path("id") roleId: Int)

    //Assign Permits
    @POST("/api/roles/{id}/assignPermit")
    suspend fun assignPermitsToRole(@Path("id") roleId: Int, @Body request: AssignPermitsRequest)

    @GET("/api/roles/{id}/permits")
    suspend fun getPermitsByRole(@Path("id") roleId: Int): List<Permit>

    @POST("/api/roles/{id}/assignPermit")
    suspend fun assignPermitsToRole(@Path("id") roleId: Int, @Body permitIds: Map<String, List<Int>>)
}